...
const labels = ['Coronavirus' ,'Health', 'Research','Food','Dental','Cardiology']
...
export function News() {
    ...
    const pagination = async () => {
        ...
        if (cards.slice(cardToRender, cardToRender + 7).length === 0)
         ...
         pagiCountrer === 0
            ? setCurrfilter(prev=> prev = cards.slice(cardToRender, cardToRender + 4))
            :setCurrfilter(prev=> prev = cards.slice(cardToRender, cardToRender + 4))
    };

    const handleSearch = (ev) => {
        const value = ev.target.value
        const filterNews = news.filter(item => item.headline.main.toLocaleLowerCase().includes(value.toLocaleLowerCase()))
        ...
        <section className="about-us-container">
            ...
            <div className="flex">
                <div className="news-card-list">
                    {currfilter.map((item,idx) => <Card key={item._id} labels={labels} item={item} RandomInt={10+idx}/> )}
                ...
                <div className="news-side-bar">
                  <div className="news-side-bar-serch">
                      ...
                      <div className="news-side-bar-serch-continer">
                          <input placeholder="Type a keyword and hit enter" onChange={handleSearch}/>
                  ...
                  <div className="news-categories-card-list">
                      {labels.map(label=>
                          <LabelCard key={label} getLabelCount={getLabelCount} label={label}/>
                        )}
                  </div>
                  </div>
                </div>
